...
def preprocess_data(stock_code):
    ...
    file_name = '/tmp/'+"daily_adjusted_" + stock_code + ".csv"
    r = requests.get(url) 
...
def get_prediction(code):
    ...
    split = 0.67
    predict_file_name ='/tmp/' + stock_code + '.csv'
...
for i in stock_warehouse:
    code=i['code']
    list['{i}'.format(i=code)]=get_prediction(code)
